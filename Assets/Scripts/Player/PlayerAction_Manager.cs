...
{
    public static PlayerAction_Manager instance;
    ...
    {
        // 크리티컬 계산
        bool isCirtical = pManager.status.criticalhit >= Random.Range(0, 100) ? true : false;

        // 데미지 계산 - 여기 레벨 벨류 어케할거 - 그냥 인덱스 추가해서 값 받아옴!
        Skill_Value_SO.Value_Data sBase = attackData.levelValue.GetData(skillLevel);
        int calDamage = (int)((sBase.type == IDamageSysteam.DamageType.Physical ? pManager.status.physicalDamage : pManager.status.magicalDamage) * Random.Range(sBase.motionValue.x, sBase.motionValue.y) * (isCirtical ? pManager.status.critical_multiplier : 1));
    ...
    {
        pManager.status.curhp += heal;
        if (pManager.status.curhp + heal > pManager.status.maxHp)
        {
