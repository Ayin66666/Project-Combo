...
// 작업중 <br />
구현 방향성
플레이어의 공격은 단일책임원칙(SRP) 공통된 호출자를 두고,<br />
호출자는 세부 내용을 알 필요 없이 호출만 하면<br />
세부 기능은 호출된 스크립트에서 동작하는 방향으로 작업하였습니다.<br />

이를 위해 인터페이스, abstract 클래스 중 어떤 뱡향으로 작업할지 고민했고, <br />
최종적으로 abstract를 통해 구현을 결정하였습니다.<br />

플레이어의 모든 공격은 Attack_Base 상속 기반 공격 시스템 구현하였습니다.
Attack_Base는 공격 타입(enum 기반 구현), 공격 콜라이더 캔슬 여부(overlapbox), 행동 간 이동 위치 데이터, 
공격 횟수나 배율에 대한 데이터를 담당합니다.
